...
for time in range(repetitions):
    ...
    for K in [100]:
        ...
        np.save('saved/MixSW_n{}_L{}_K{}_repeat{}.npy'.format(n, L, K, time), matrices)
        ...
        np.savetxt('saved/Zs_MixSW_n{}_L{}_K{}_repeat{}.txt'.format(n, L, K, time), Z.reshape(1, -1), delimiter=',',
                   fmt="%d")
        print(np.unique(Z))
        ...
        np.save('saved/MixSW_TV_n{}_L{}_K{}_repeat{}.npy'.format(n, L, K, time), np.array(TV))
        np.save('saved/MixSW_SW_n{}_L{}_K{}_repeat{}.npy'.format(n, L, K, time), np.array(W2))
        np.save('saved/MixSW_trueTV_n{}_L{}_K{}_repeat{}.npy'.format(n, L, K, time), np.array(trueTV))
        np.save('saved/MixSW_trueSW_n{}_L{}_K{}_repeat{}.npy'.format(n, L, K, time), np.array(trueW2))
