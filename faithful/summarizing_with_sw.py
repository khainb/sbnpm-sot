    ...
    for j in range(i + 1, N):
        with torch.no_grad():
            matrices[i, j] = SW(torch.cat([mu2[i], Sigma2[i].view(Sigma2[i].shape[0], -1)], dim=1),
                                torch.cat([mu2[j], Sigma2[j].view(Sigma2[j].shape[0], -1)], dim=1), pi2[i], pi2[j], L=L)
            if (np.isnan(matrices[i, j])):
                print('Nan')
            matrices[j, i] = matrices[i, j]

np.save('saved/SW_L{}.npy'.format(L), matrices)
...
np.savetxt('saved/Zs_SW_L{}.txt'.format(L), Z.reshape(1, -1), delimiter=',', fmt="%d")
print(np.unique(Z))
...
np.save('saved/SW_TV_L{}.npy'.format(L), np.array(TV))
np.save('saved/SW_SW_L{}.npy'.format(L), np.array(W2))
